...
def fov2tan_pinhole(fovx, fovy, interval):
    num_theta = int(np.floor(fovx / interval))
    num_phi = int(np.floor(fovy / interval))
    tan_theta_vals = np.linspace(-np.tan(fovx), np.tan(fovx), num=2 * num_theta)
    tan_phi_vals = np.linspace(-np.tan(fovy), np.tan(fovy), num=2 * num_phi)
    theta_arr = np.arctan(tan_theta_vals)
    phi_arr = np.arctan(tan_phi_vals)
...
def colmap_main(args):
    ...
    tan_theta, tan_phi = fov2tan_pinhole(FoVx, FoVy, args.step)
