...
def training(args, dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, mask_path=None):
    ...
    for iteration in range(first_iter, opt.iterations + 1):      
        # random background
        if args.train_random_background: 
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                postfix = {
                    "Loss": Fore.YELLOW+f"{ema_loss_for_log:.{5}f}"+Style.RESET_ALL, 
                    "psnr": Fore.CYAN+f"{ps.item():.2f}"+Style.RESET_ALL,
                    "Num": Fore.RED+f"{len(gaussians._xyz)}"+Style.RESET_ALL
                }
...
if __name__ == "__main__":
    ...
    print(Fore.GREEN+"Optimizing " + args.model_path + Style.RESET_ALL)


    args.port = randint(8000, 9000)
